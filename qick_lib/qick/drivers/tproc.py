...
class Axis_QICK_Proc(SocIp):
    ...
    def reload_program(self):
        """Reload the program memory."""
        try:
            self.Load_PMEM(self.binprog)
        except AttributeError as err:
            raise RuntimeError(f'Could not reload the program because the program has not yet been loaded.') from err
